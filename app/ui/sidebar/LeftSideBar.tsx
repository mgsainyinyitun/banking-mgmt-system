...
import { DEFAULT_PROFILE, ROUTE_TYPE } from '@/app/constants/CONSTANTS';
                    ...
                    <nav className='mt-10'>
                        {
                            customerSideBar.map(itm => (
                                <Link href={`/${ty ? ROUTE_TYPE[ty] : ''}${itm.link}`}
                                    key={itm.name}
                                    className={`flex py-2.5 px-4 rounded-2xl transition duration-200 hover:bg-primary-300 text-lg hover:text-white hover:font-semibold`}
                                >
